...
// Verinin formatını değiştiriyorum
const formattedData = data.map(person => ({
    fullName: `${person.first_name} ${person.last_name || ''}`,
    username: person.username,
    power: parseInt(person.power, 10),
    birthDate: new Date(person.birth_date.split('.').reverse().join('-'))
}));

// ortalamayı hesapladığım kısım
const averagePower = formattedData.reduce((acc, curr) => acc + curr.power, 0) / formattedData.length;

// Sonuçları konsola yazdırıyorum.
